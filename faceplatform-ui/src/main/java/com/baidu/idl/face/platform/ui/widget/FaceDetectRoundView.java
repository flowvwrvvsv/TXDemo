...
public class FaceDetectRoundView extends View
{
    ...
    public static final float WIDTH_SPACE_RATIO = 0.22f;
    ...
    public static int COLOR_BG = Color.parseColor("#99000000");
    ...
    private Paint mPathComparePaint;
    ...
    private DashPathEffect mCompareDashOut;
    private DashPathEffect mCompareDashInner;
    private Handler mHandler;
    private boolean isCompared = false;
    private String comparedStr = "对比中";
    private TextPaint mTextPaint;

    public void startCompare()
    {
        this.isCompared = true;
        mBGPaint.setColor(Color.parseColor("#00000000"));
        mTextPaint = new TextPaint();
        mTextPaint.setColor(Color.parseColor("#FFFFFF"));
        mTextPaint.setTextSize(DensityUtils.dip2px(context, 34));
        mTextPaint.setStyle(Paint.Style.FILL);
        mTextPaint.setAntiAlias(true);
        mTextPaint.setDither(true);
        mPathComparePaint = new Paint();
        mPathComparePaint.setColor(COLOR_ROUND);
        mPathComparePaint.setStrokeWidth(DensityUtils.dip2px(context, 6));
        mPathComparePaint.setStyle(Paint.Style.STROKE);
        mPathComparePaint.setAntiAlias(true);
        mPathComparePaint.setDither(true);
        ...
        mHandler = new Handler()
        {
            public void handleMessage(Message m)
            {
                ...
                float pathSpaceOut = DensityUtils.dip2px(context, 24);
                float pathSmallSpaceOut = DensityUtils.dip2px(context, PATH_SMALL_SPACE);

                float pathSpaceInner = DensityUtils.dip2px(context, 12);
                float pathSmallSpaceInner = DensityUtils.dip2px(context, 6);

                mCompareDashOut = new DashPathEffect(
                        new float[]{pathSpaceOut, dm.heightPixels < SURFACE_HEIGHT
                                ? pathSmallSpaceOut : pathSpaceOut}, m.what + 250);

                mCompareDashInner = new DashPathEffect(
                        new float[]{pathSpaceInner, pathSmallSpaceInner}, m.what * -1);

                postInvalidate();
                mHandler.sendEmptyMessageDelayed(m.what + 200, 500);
            }
        };
    ...
    public int getCycleBottom()
    {
        return (int) (getHeight() - (mY + mR)) - 10;
    }
    ...
    public void onDraw(Canvas canvas)
    {
        ...
        if (isCompared)
        {
            canvas.drawPaint(mBGPaint);
            mPathComparePaint.setStrokeWidth(DensityUtils.dip2px(context, 6));
            mPathComparePaint.setPathEffect(mCompareDashOut);
            canvas.drawCircle(mX, mY, mR, mPathComparePaint);

            mPathComparePaint.setStrokeWidth(DensityUtils.dip2px(context, 2));
            mPathComparePaint.setPathEffect(mCompareDashInner);
            canvas.drawCircle(mX, mY, mR - (mR / 6), mPathComparePaint);
            int widthStr = -1;
            Rect rect = new Rect();
            mTextPaint.getTextBounds(comparedStr, 0, comparedStr.length(), rect);//用一个矩形去"套"字符串,获得能完全套住字符串的最小矩形
            widthStr = rect.width();//字符串的宽度
            canvas.drawText(comparedStr, mX - widthStr / 2, mY - 20, mTextPaint);

            String dianStr = "....";
            mTextPaint.getTextBounds(dianStr, 0, dianStr.length(), rect);//用一个矩形去"套"字符串,获得能完全套住字符串的最小矩形
            widthStr = rect.width();//字符串的宽度
            canvas.drawText(dianStr, mX - widthStr / 2, mY + mR / 6, mTextPaint);
        } else
        {
            canvas.drawPaint(mBGPaint);
            if (mIsDrawDash)
